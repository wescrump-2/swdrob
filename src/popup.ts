...
OBR.onReady(async () => {
  ...
  if (metadata?.character && metadata.timestamp) {
    ...
    saved = DEFAULT_STATBLOCK;
  }

  // Convert old format with rolls to new format with attributes/skills if needed
  const char = saved as any;
  if (char.rolls) {
    const attributes = ['agility', 'smarts', 'spirit', 'strength', 'vigor'].map(attr => ({
      name: attr,
      die: char.rolls[attr] || 'd6'
    }));
    const skills = Object.entries(char.rolls).filter(([k]) => !['agility', 'smarts', 'spirit', 'strength', 'vigor'].includes(k)).map(([k, v]: [string, any]) => ({
      name: k,
      die: v
    }));
  ...
  function addRollHandlers() {
    document.querySelectorAll('.popup-roll-btn').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        const dieStr = target.dataset.die || 'd6';
  ...
  document.getElementById("import-url")!.onclick = async () => {
    try {
      const char = await Savaged.parseCharacterFromURL(urlInput.value);
      // Save URL and character to item metadata
      await OBR.scene.items.updateItems([itemId], (items) => {
        for (const item of items) {
          item.metadata[Util.StatBlockMkey] = { url: urlInput.value, character: char, timestamp: Date.now() };
        }
      });
      await applyData(char);
      //urlInput.value = "";
    } catch (e) {
      alert("Failed to load from URL");
      await applyData(DEFAULT_STATBLOCK);
    }
  };


  function extractFormData(): Character {
  ...
  async function applyData(data: Partial<Character>) {
    saved = { ...DEFAULT_STATBLOCK, ...data };
    await OBR.scene.items.updateItems([itemId!], (items) => {
      for (const item of items) {
        const existing = item.metadata[Util.StatBlockMkey] as any;
        item.metadata[Util.StatBlockMkey] = { ...existing, character: saved, timestamp: Date.now() };
      }
    });
    // Re-populate form
    populateForm(saved);
